...
import {
  getBackendColorPaletteService,
  getPredefinedPalette,
  type ColorPalette,
} from "@/app/utils/colorPalette";
...
export function useColorPalette() {
  ...
  const generateColorPalette = useCallback(
    async (request: ColorPaletteRequest): Promise<ColorPalette | null> => {
      setIsLoading(true);
      setError(null);
  ...
  const generateColorPaletteOptions = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Failed to generate color palette options";
        setError(errorMessage);
        setIsLoading(false);
        return [];
      }
  ...
  const generatePaletteForSlide = useCallback(
    async (
      slideContent: string,
      slideType?: string,
